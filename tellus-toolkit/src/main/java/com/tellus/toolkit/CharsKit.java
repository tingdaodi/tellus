...
public class CharsKit {
     ...
     */
    public static String nullToDefault(@Nullable String str, String defaultStr) {
     ...
     * @return 如果是set或get方法名，返回field， 否则 Optional.empty()
     */
    public static Optional<String> getGeneralField(String getOrSetMethodName) {
     ...
     */
    public static Optional<String> cutPreAndLowerFirst(String str, int preLength) {

        if (str.length() > preLength) {
            char first = Character.toLowerCase(str.charAt(preLength));
            if (str.length() > preLength + 1) {
                return Optional.of(first + str.substring(preLength + 1));
            }
     ...
     * @return bytes
     */
    public static byte[] getBytes(String str, @Nullable Charset charset) {
     ...
     * @return 格式化后的文本
     */
    public static String format(String template, @Nullable Map<?, ?> map) {
     ...
     * @return 编码后的字节码
     */
    public static byte[] bytes(String str, @Nullable String charset) {
     ...
     * @return 编码后的字节码
     */
    public static byte[] bytes(String str, @Nullable Charset charset) {
        if (null == charset) {
            return str.getBytes();
        }
     ...
     * @return 解码后的字符串
     */
    public static String str(byte[] data, @Nullable Charset charset) {
        if (null == charset) {
     ...
     * @return 字符串
     */
    public static String str(ByteBuffer data, @Nullable Charset charset) {
        if (null == charset) {
    ...
    public static ByteBuffer byteBuffer(String str, String charset) {
        return ByteBuffer.wrap(CharsKit.bytes(str, charset));
    }
     ...
     * @return 编码后的字节码
     */
    public static byte[] encode(String str, @Nullable String charset) {
        if (isBlank(charset)) {
            return str.getBytes();
        }
        try {
     ...
     * @return 解码后的字符串
     */
    public static String decode(byte[] data, @Nullable String charset) {
        if (isBlank(charset)) {
            return new String(data);
        }
        try {
    ...
    public static String convert(String source, String srcCharset, String newCharset) {
        if (CharsKit.isBlank(srcCharset)) {
            srcCharset = ISO_8859_1;
        }

        if (CharsKit.isBlank(newCharset)) {
            srcCharset = UTF_8;
        }

        if (CharsKit.isBlank(source) || srcCharset.equals(newCharset)) {
            return source;
        }
        try {
     ...
     * @return 字符串
     */
    public static String str(ByteBuffer data, @Nullable String charset) {
        Charset cs;

        if (CharsKit.isBlank(charset)) {
     ...
     * @return byteBuffer
     */
    public static ByteBuffer toByteBuffer(String str, @Nullable String charset) {
        return ByteBuffer.wrap(CharsKit.encode(str, charset));
    }
